...
namespace Gimp
  ...
  public class Path
    ...
    public Path(Int32 imageID, string name)
      {
      _imageID = imageID;
      _name = name;
      }
    ...
    public void ToSelection(ChannelOps op, bool antialias,
                            bool feather, double feather_radius_x, double feather_radius_y)
      {
    ...
    public void ImportString(string importString, int length, bool merge,
                             bool scale)
      {
      if (!gimp_path_import_string(_imageID, importString, length, merge, scale))
        {
        throw new Exception();
        }
      }
